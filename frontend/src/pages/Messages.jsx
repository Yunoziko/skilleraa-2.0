...
export default function Messages() {
  ...
  const active = useMemo(
    () => conversations.find((c) => c.id === activeId) || getConversation(activeId),
    [conversations, activeId],
  );

  const refreshList = () => {
    setConversations(listConversations(query));
    setLoading(false);
  };

  const refreshThread = (id) => {
    if (!id) {
      setMessages([]);
      return;
    }
            ...
            {loading ? (
              ...
              <div className="p-6">
                <EmptyState
                  ...
                  description="Search returned no matches, or your inbox is empty."
                  icon={MessageSquare}
                />
              </div>
            ) : (
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {c.unread > 0 && (
                          <span className="text-[10px] font-semibold bg-black text-white rounded-full min-w-[18px] h-[18px] grid place-items-center px-1">
                            {c.unread}
                          </span>
                        )}
                      </div>
          ...
          {!active ? (
            ...
            <>
              <header className="px-5 py-4 border-b skl-border flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-medium">{active.participant_name}</div>
                  <div className="text-[10px] uppercase tracking-widest text-neutral-500 font-semibold">
                    {active.participant_role}
                  </div>
                </div>
              </header>
